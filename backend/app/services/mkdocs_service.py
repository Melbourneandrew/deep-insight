...
class MkDocsNavService:
    ...
    def _load_config(self) -> None:
        ...
        if not self.mkdocs_path.exists():
            raise FileNotFoundError(f"mkdocs.yml not found at {self.mkdocs_path} (absolute: {self.mkdocs_path.absolute()})")
        ...
        with open(self.mkdocs_path, "r", encoding="utf-8") as f:
            try:
                ...
                logger.warning("Failed to load with unsafe_load, trying safe_load: %s", e)
    ...
    def update_nav_from_json(self, sections_json: dict[str, Any]) -> None:
        """
