        ...
        try:
            ...
            try:
                ...
                if hasattr(response, "choices") and response.choices:  # type: ignore
                    ...
                    if hasattr(choice, "message") and choice.message:  # type: ignore
                        ...
                        if (
                            ...
                            logger.info(
                                f"Successfully generated follow-up question: {content[:100]}..."
                            )
                            return content
                        # Try reasoning_content if content is empty (for reasoning models)
                            ...
                            for pattern in question_patterns:
                                question_match = re.search(
                                    pattern, reasoning, re.IGNORECASE
                                )
                                if question_match:
                                    question = question_match.group(1).strip()
                                    if len(question) > 10 and (
                                        "?" in question or question.endswith(".")
                                    ):  # Basic validation
                                        logger.info(
                                            f"Extracted question from reasoning: {question[:100]}..."
                                        )
                                        return question

                            logger.error(
                                f"Could not extract valid question from reasoning: {reasoning[:200]}..."
                            )
                            raise ValueError(
                                "Failed to parse LLM response - no valid content found"
                            )

                # Method 2: Try dict-like access on response object
                try:
                    if hasattr(response, "__dict__") or hasattr(
                        ...
                        if choices and len(choices) > 0:
                            ...
                            if message:
                                ...
                                if content and str(content).strip():
                                    logger.info(
                                        f"Successfully generated follow-up question: {str(content)[:100]}..."
                                    )
                ...
                if content_match:
                    ...
                    if (
                        content
                        and not content.startswith("We need to generate")
                        and not content.startswith("The user wants")
                    ):
                        logger.info(
                            f"Successfully extracted question from string: {content[:100]}..."
                        )
                        return content
                    else:
                        logger.warning(
                            f"Filtered out invalid question content: {content[:100]}..."
                        )
                ...
                if content_match:
                    ...
                    if (
                        ...
                        logger.info(
                            f"Successfully extracted question from JSON pattern: {content[:100]}..."
                        )
                        return content

                # Log detailed debug info
                logger.error(
                    f"Unable to parse LLM response. Response type: {type(response)}"
                )
                logger.error(
