...
from app.agents.prompts.write_doc import WRITE_DOC_PROMPT, WRITE_DOC_USER_PROMPT
...
async def plan_documentation_sections(
    yaml_text: str, model: str = "openai/gpt-5-mini-2025-08-07"
) -> dict:
    """
    ...
    """
    logger.info(
        "Planning documentation sections",
        extra={"model": model, "yaml_chars": len(yaml_text)},
    )
    ...
    logger.info(
        ...
        extra={"sections_count": len(parsed_response.get("sections", []))},
    )
...
async def write_documentation_content(
    yaml_text: str,
    title: str,
    section_name: str,
    ...
    async with semaphore if semaphore else asyncio.nullcontext():
        logger.info(
            ...
            extra={"title": title, "section": section_name, "model": model},
        )

        user_prompt = WRITE_DOC_USER_PROMPT.format(
            title=title, section_name=section_name, yaml_text=yaml_text
        )
...
async def _create_single_doc(
    doc_info: dict, yaml_text: str, model: str, semaphore: asyncio.Semaphore
) -> Path:
    ...
    content = await write_documentation_content(
        yaml_text, title, section_name, model, semaphore
    )
...
async def create_documentation_files(
    sections_json: dict,
    ...
    logger.info(
        "Creating %d documentation files concurrently (max 10 at a time)",
        len(docs_to_create),
    )
...
def update_mkdocs_navigation(
    sections_json: dict, docs_root_dir: Path, business_name: str = None
) -> None:
    """
    ...
    Args:
        ...
        business_name: Optional business name to set as site_name
    """
    mkdocs_service = create_mkdocs_nav_service(docs_root_dir)
    mkdocs_service.update_nav_from_json(sections_json, site_name=business_name)
...
async def run_chain(
    ...
    Args:
        ...
        docs_root_dir: Path to the docs directory containing mkdocs.yml
        model: The LLM model to use
        business_name: Optional business name to set as site_name in mkdocs.yml
    ...
    update_mkdocs_navigation(sections_plan, docs_root_dir, business_name)

    # Step 3: Create documentation files with generated content (async with semaphore)
    created_files = await create_documentation_files(
        sections_plan, docs_output_dir, yaml_text, model
    )

    logger.info(
        "Documentation chain completed",
        extra={
